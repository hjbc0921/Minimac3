...
struct saxpy_functor
{
    ...
    __host__ __device__
        double operator()(const int& x, const float& y) const { 
            return a * x + y;
        }
};
                            ...
                            double reco,vector<int> &uniqueCardinality)
{
    ...
    thrust::device_vector<float> f(from.begin(), from.end());
    thrust::device_vector<float> t(to.begin(), to.end());
    thrust::device_vector<int> u(uniqueCardinality.begin(), uniqueCardinality.end());
    thrust::transform(f.begin(), f.end(), t.begin(), t.begin(), saxpy_functor(complement)); 
    thrust::transform(u.begin(), u.end(), t.begin(), t.begin(), saxpy_functor(sum)); 
    thrust::copy(t.begin(), t.end(), to.begin());
